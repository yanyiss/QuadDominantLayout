...
#include <iostream>
void triangulate(const Eigen::Matrix2Xd &all_pts, const Eigen::Matrix2Xi &E, const double area_threshold, Mesh &mesh)
{
	...
	{
		vhandle.push_back(mesh.add_vertex(Mesh::Point(out.pointlist[2 * i], out.pointlist[2 * i + 1], 0)));
	}
...
void triangulate(const Eigen::Matrix2Xd &all_pts, const std::vector<Eigen::Matrix2Xi> &E, const double area_threshold, Mesh &mesh)
{
	if (E.size() == 1)
	{
